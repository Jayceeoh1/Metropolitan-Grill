...
export default function AdminSetariPage() {
  const [settings, setSettings] = useState({
    ...
    address: 'Str. Mihai Eminescu 42, Sector 2, București',
  ...
  const save = () => toast.success('Setări salvate cu succes!')

  return (
    <div className="page-enter">
      <h1 className="font-bebas text-4xl text-white mb-6">SETĂRI RESTAURANT</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={f.key}>
                ...
                <input type={f.type || 'text'} className={inputClass} value={settings[f.key]} onChange={e => setSettings(s => ({ ...s, [f.key]: e.target.value }))} />
              </div>
            ))}
            <button onClick={save} className="w-full bg-gradient-to-r from-[#c0392b] to-[#96251e] hover:from-[#e74c3c] hover:to-[#c0392b] text-white font-condensed font-bold text-sm uppercase tracking-wide py-3 rounded-xl transition-all mt-2">
              Salvează Date
            </button>
          </div>
        </div>
        ...
        <div className="bg-[#1a1a1a] border border-white/8 rounded-[18px] p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wide text-[#7a6e66] mb-1.5">Sâmbătă – Duminică</label>
            ...
            <div>
              ...
              <select className={inputClass} value={settings.status} onChange={e => setSettings(s => ({ ...s, status: e.target.value }))}>
                ...
                <option value="vacation">🏖 În Vacanță</option>
              </select>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-bold uppercase tracking-wide text-[#7a6e66] mb-1.5">Comandă Minimă (lei)</label>
                <input type="number" className={inputClass} value={settings.minOrderDelivery} onChange={e => setSettings(s => ({ ...s, minOrderDelivery: Number(e.target.value) }))} />
              </div>
              <div>
                <label className="block text-xs font-bold uppercase tracking-wide text-[#7a6e66] mb-1.5">Timp Preparare (min)</label>
                <input type="number" className={inputClass} value={settings.preparationTime} onChange={e => setSettings(s => ({ ...s, preparationTime: Number(e.target.value) }))} />
              </div>
            </div>
            <button onClick={save} className="w-full bg-gradient-to-r from-[#c0392b] to-[#96251e] hover:from-[#e74c3c] hover:to-[#c0392b] text-white font-condensed font-bold text-sm uppercase tracking-wide py-3 rounded-xl transition-all mt-2">
              Salvează Program
